...
public class MiddleEnemy : CEnemyMove
{
    ...
    void Start()
    {
        ...
        UIManager.Instance.BossName.text = "준보스";
        m_GetStat.m_HPBar = UIManager.Instance.BossHP;
...
//        m_EnemyAnim.SetBool("IsMove", false);
    ...
    protected override void SetTrace()
    {
        ...
        //        m_EnemyAnim.SetBool("IsMove", false);
        if (!m_SkillStartCheck)
        {
            StartCoroutine(GetComponent<EnemySkill>().MiddleSkillPlay());
            GetComponent<EnemySkill>().SkillNum = 0;
            m_SkillStartCheck = true;
        }
    ...
    void OnSkillAttack()
    {
        if (GetComponent<EnemySkill>().SkillCheck)
        {
            float TargetDist = m_TargetDist;
            //                float AttackDist = m_Move.GetComponent<EnemyMove>().m_RangeDist;
            if (TargetDist <= 2.5f)
            {
                GetComponent<EnemySkill>().SkillNum = 2;
            }
            else if (TargetDist <= 8)
            {
                GetComponent<EnemySkill>().SkillNum = 1;
